...
class Function:
    ...
    def to_c_prototype(self, type_cache: dict[t.Type, tuple[str, str]]) -> str:
        return f"{self.comment_line}EXTERN {self.__prototype(type_cache)};\n"

    def to_c_implement(self, type_cache: dict[t.Type, tuple[str, str]]) -> str:
        return (f"{self.comment_line}EXPORT {self.__prototype(type_cache)}\n"
                f"{{\n"
        ...
        if self.object_name and self.init:
            ...
            if isinstance(last_element, p.InitArray):
                return (f"static struct {{\n"
                        ...
                        f"}}[1] {mangle_name(self.name)}")
            return f"static {mangle_name(self.object_name)}_t[1] {mangle_name(self.name)}"
        return f"static {self.type.declare(type_cache)} {mangle_name(self.name)}"
    ...
    def to_c_implement(self, type_cache: dict[t.Type, tuple[str, str]]) -> str:
        ...
        if self.object_name:
            if not isinstance(self.init, p.NewStruct):
                raise ValueError("init must be NewStruct")
            return (f"{self.__prototype(type_cache)} = {{\n"
             f"    (const vtable_t const *)&obj_{mangle_name(self.name)}\n"
