                ...
                if (op.result != registerToExclude && objectFields.isNotEmpty()) {
                    ...
                    listOf(op) + objectFields.flatMap { fieldPath ->
                        ...
                        if (fieldPath.isNotEmpty()) {
                            ...
                            val store = CgOp.Store(
                                CgTypePrimitive.OBJECT,
                                CgValue.Register("${op.result.name}.arc$pathStr", CgTypePointer(from.type)),
                                from
                            )
                            ...
                            val store = CgOp.Store(
                                CgTypePrimitive.OBJECT,
                                CgValue.Register("${op.result.name}.arc", CgTypePointer(op.result.type)),
                                op.result
                            )
                            listOf(store)
                        }
            ...
            is CgOp.Return -> {
                ...
                val acquires = if (op.returnValue != registerToExclude && objectFields.isNotEmpty()) {
                    ...
                    objectFields.flatMap { fieldPath ->
                        val pathStr = fieldPath.joinToString("$")
                        if (fieldPath.isNotEmpty()) {
                            val from = CgValue.Register(
                                (op.returnValue as CgValue.Register).name + pathStr,
                                CgTypePrimitive.OBJECT
                            )
