...
import com.supersoftcafe.yafl.passes.searchAndReplaceExpressions
...
class LambdaToClass : AbstractUpdater<List<Declaration>>(emptyList(), { l, r -> l+r }) {
    ...
    override fun updateExpressionLambda(
        ...
        val localParams = path.firstNotNullOfOrNull { it as? Declaration.Function }?.let { it.parameter.flatten() + it.thisDeclaration } ?: listOf()
...
fun lambdaToClass(ast: Ast): Ast {
    val ast2 = ast.copy(
        declarations = ast.declarations.map { root ->
            root.copy(
                // Any new declarations as a result of lowering lambdas must be kept within the
                // same 'Root' context so that future generics support is kept simpler.
                declarations = root.declarations.flatMap { declaration ->
                    val (result, additional) = LambdaToClass().updateDeclaration(declaration, listOf())
                    additional + result
                }
            )
        }
