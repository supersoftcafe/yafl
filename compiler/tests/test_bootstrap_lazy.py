"""bootstrap lazy stage — the post-monomorphisation driver segment must
produce the same AST as compiler.py:588-624: the literals segment, then the [lazy] forward-ref check and the deferred-init lowering (auto-promotion of non-static globals, thunk wrapping, PeLazy reference rewriting) — lambda_lift
...
class TestBootstrapLazy(TestCase):
    _TIMEOUT = 2400
    ...
    def test_lazy_ast_matches_python(self):
        for path in _CORPUS:
            with self.subTest(file=path.name):
                ...
                for i, (e, gg) in enumerate(zip(expected, got)):
                    self.assertEqual(e, gg, f"{path.name}: lazy AST "
                                            f"differs at line {i + 1}")
                self.assertEqual(len(expected), len(got),
                                 f"{path.name}: lazy AST length differs")
