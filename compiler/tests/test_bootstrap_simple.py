"""bootstrap simple stage — the post-monomorphisation driver segment must
produce the same AST as compiler.py:588-626: the lambdas segment, then simple-class flattening — qualifying classes become flat field tuples with methods lifted to Cls__method free functions — lambda_lift
...
class TestBootstrapSimple(TestCase):
    _TIMEOUT = 2400
    ...
    def test_simple_ast_matches_python(self):
        for path in _CORPUS:
            with self.subTest(file=path.name):
                ...
                for i, (e, gg) in enumerate(zip(expected, got)):
                    self.assertEqual(e, gg, f"{path.name}: simple AST "
                                            f"differs at line {i + 1}")
                self.assertEqual(len(expected), len(got),
                                 f"{path.name}: simple AST length differs")
