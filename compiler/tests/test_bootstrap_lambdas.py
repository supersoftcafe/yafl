"""bootstrap lambdas stage — the post-monomorphisation driver segment must
produce the same AST as compiler.py:588-625: the lazy segment, then closure conversion — every lambda lifts to a global function, capturing ones to a [final] class whose fields are the captures (untyped captures stay untyped), with paths disambiguating monomorphised copies — lambda_lift
...
class TestBootstrapLambdas(TestCase):
    _TIMEOUT = 2400
    ...
    def _python_lambdas(self, text: str) -> str:
        from tests.testutil import cached_reference
        return cached_reference("lambdas", text,
                                lambda: self._python_lambdas_uncached(text))

    def _python_lambdas_uncached(self, text: str) -> str:
    ...
    def test_lambdas_ast_matches_python(self):
        for path in _CORPUS:
            with self.subTest(file=path.name):
                ...
                for i, (e, gg) in enumerate(zip(expected, got)):
                    self.assertEqual(e, gg, f"{path.name}: lambdas AST "
                                            f"differs at line {i + 1}")
                self.assertEqual(len(expected), len(got),
                                 f"{path.name}: lambdas AST length differs")
