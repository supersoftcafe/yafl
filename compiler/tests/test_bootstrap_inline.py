"""bootstrap inline stage — the post-monomorphisation driver segment must
produce the same AST as compiler.py:588-612: the tail segment, then inline_ast — lambda_lift
...
class TestBootstrapInline(TestCase):
    _TIMEOUT = 2400
    ...
    def test_inline_ast_matches_python(self):
        for path in _CORPUS:
            with self.subTest(file=path.name):
                ...
                for i, (e, gg) in enumerate(zip(expected, got)):
                    self.assertEqual(e, gg, f"{path.name}: inline AST "
                                            f"differs at line {i + 1}")
                self.assertEqual(len(expected), len(got),
                                 f"{path.name}: inline AST length differs")
